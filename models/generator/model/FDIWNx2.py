...
from model.common import MeanShift, std, default_conv, CEALayer, default_conv_stride2
...
class CoffConv(nn.Module):###zhege mokuai de zuoyong shi shenme
...
class MY(nn.Module):####zhege shi WDIB de daima
    ...
    def forward(self, x):
        ...
        out_b_1 = A1 * out_a + x ####  v1

        B1 = self.B1_coffconv(x)
        out_a_1 = B1 * x + out_a ### u1
        ####
        ...
        out_b_2 = A2 * out_a_1 + out_b ### v2
        out_b_2 = out_b_2 * self.sigmoid1(self.conv_distilled1(distilled_b1))
        B2 = self.B2_coffconv(out_b)
        out_a_2 = out_b * B2 + out_a_1 ##### u2
        out_a_2 = out_a_2 * self.sigmoid2(self.conv_distilled2(distilled_a1)) ##

        out_a_out = self.srb3(out_a_2)#self.conv3_a(out_a_2) + out_a_2 ### xout1
        out_b_out = self.srb4(out_b_2)#self.conv3_b(out_b_2) + out_b_2 ### xout2
##### houmian de daima youdian kanbudong?????
        out1 = self.fuse1(torch.cat([self.scale_x1(out_a_out), self.scale_x2(out_b_out)], dim=1)) ##fuse1==scf
...
class Li(nn.Module):##zheshi FSWG
    def __init__(self, n_feats, bn=False, act=nn.ReLU(True)):
        super(Li, self).__init__()

        self.act = activation('lrelu', neg_slope=0.05)
        wn = lambda x: torch.nn.utils.weight_norm(x)
        self.MY1 = MY(n_feats ) ##jiushi WDIB de daima #WDIB
        self.MY2 = MY(n_feats )
        self.MY3 = MY(n_feats )
        ...
        self.channel_shuffle1 = ShuffleBlock(groups=4) ##chachengzu  ranhou zushang
...
class MODEL(nn.Module):
    ...
    def __init__(self, conv=default_conv):
        ...
        self.Upsample = nn.Upsample(scale_factor=4,
                                    mode='bilinear', align_corners=False) #shangcaiyang sibei
        rgb_mean = (0.4488, 0.4371, 0.4040)
        rgb_std = (1.0, 1.0, 1.0)
        self.Sub_mean = MeanShift(255, rgb_mean, rgb_std)
        self.Add_mean = MeanShift(255, rgb_mean, rgb_std, 1)
        self.head = conv(n_colors, 3*n_feats//8 , kernel_size)  #jiushi juanji
        self.Li1 = Li(3*n_feats//8) # 3*64//8
    ...
    def forward(self, x):
        y_input = self.Sub_mean(x) #duiqi qiujunzhi
        y_input = self.head(y_input)  # juanji caozuo
        y_input = self.Li1(y_input) ### zheshi FSWG de daima
        ...
        ###zuihou yiceng jiushi dou jiashang qu shishi
